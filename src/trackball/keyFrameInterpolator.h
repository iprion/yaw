...
class KeyFrameInterpolator : public Signaler  {
...
public:
  ...
  std::function<void(void)> onEndReachedCallBack() const { return onEndReachedCallBack_; }
  std::function<void(void)> onInterpolatedCallBack() const { return onInterpolatedCallBack_; }
  ...
  void setOnEndReachedCallBack(std::function<void(void)> onEndReachedCallBack) { onEndReachedCallBack_ = onEndReachedCallBack; }
  void setOnInterpolatedCallBack(std::function<void(void)> onInterpolatedCallBack) { onInterpolatedCallBack_ = onInterpolatedCallBack; }

  /*! @name Path creation */
  //@{
public: 
  void addKeyFrame(Frame &frame);
  void addKeyFrame(Frame &frame, qreal time);
  void addKeyFrame(Frame *frame);
  void addKeyFrame(Frame *frame, qreal time);
...
private:
  ...
  // K e y F r a m e s
  mutable std::list<KeyFrame *> keyFrame_;
  std::vector<std::list<KeyFrame *>::iterator> currentFrame_;
  std::list<Frame> path_;
  ...
  std::function<void(void)> onEndReachedCallBack_ = nullptr;
  std::function<void(void)> onInterpolatedCallBack_ = nullptr;
};
