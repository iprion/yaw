...
public:
  ...
  void interpolateTo(Frame &fr, qreal duration);
  //@}
...
public:
  ...
  const std::map<unsigned int, KeyFrameInterpolator *>& kfi() {
    return kfi_;
  }

  KeyFrameInterpolator * interpolationKfi() {
    return interpolationKfi_;
  }
...
private:
  ...
  std::map<unsigned int, KeyFrameInterpolator *> kfi_;
