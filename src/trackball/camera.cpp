...
Camera::Camera(const Camera &camera) : frame_(nullptr) {
...
Camera &Camera::operator=(const Camera &camera) {
  ...
  //frame_->connect("modified", std::bind(&Camera::onFrameModified, this), this);
...
void Camera::setFrame(ManipulatedCameraFrame *const mcf) {
  ...
  if (frame_) {
    frame_->disconnect("modified", this);    
  }
...
void Camera::loadProjectionMatrixStereo(bool leftBuffer) const {
  ...
  case Camera::ORTHOGRAPHIC:
    std::cerr << "Camera::setProjectionMatrixStereo: Stereo not available with Ortho mode" << std::endl;
    break;
  }
}
...
void Camera::interpolateToZoomOnPixel(const QPoint &pixel) {
  ...
  interpolationKfi_->addKeyFrame(
      new Frame(0.3 * frame()->position() + 0.7 * target, frame()->orientation()),
      0.4);
...
void Camera::interpolateTo(Frame &fr, qreal duration) {
  if (interpolationKfi_->interpolationIsStarted())
    interpolationKfi_->stopInterpolation();
...
void Camera::fitBoundingBox(const Vec &min, const Vec &max) {
  qreal diameter = std::max(fabs(max[1] - min[1]), fabs(max[0] - min[0]));
...
void Camera::drawAllPaths() {
  for (std::map<unsigned int, KeyFrameInterpolator *>::const_iterator
           it = kfi_.begin(),
           end = kfi_.end();
       it != end; ++it)
    (it->second)->drawPath(3, 5, sceneRadius());
}
