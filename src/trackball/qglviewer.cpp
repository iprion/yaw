...
void QGLViewer::defaultConstructor() {
  ...
  // std::cerr << "Unable to get OpenGL version, context may not be available -
  ...
  messageTimer_.connect("timeout", std::bind(&QGLViewer::hideMessage, this), this);
  delayedFullScreenTimer_.connect("timeout", std::bind(&QGLViewer::delayedFullScreen, this), this);
  resetVisualHintsTimer_.connect("timeout", std::bind(&QGLViewer::resetVisualHints, this), this);
...
QGLViewer::QGLViewer() : Signaler({"axisIsDrawnChanged", "gridIsDrawnChanged","FPSIsDisplayedChanged", "textIsEnabledChanged", "animateNeeded", "viewerInitialized","helpRequired" }) {
  defaultConstructor();
}
...
static std::string QGLViewerVersionString() {
  return std::to_string((QGLVIEWER_VERSION & 0xff0000) >> 16) + "." +
         std::to_string((QGLVIEWER_VERSION & 0x00ff00) >> 8) + "." +
         std::to_string(QGLVIEWER_VERSION & 0x0000ff);
}

static Qt::KeyboardModifier keyboardModifiersFromState(unsigned int state) {
  // Convertion of keyboard modifiers and mouse buttons as an int is no longer
  // supported : emulate
  return Qt::KeyboardModifier(int(state & 0xFF000000));
}
...
void QGLViewer::initializeGL() {
  ...
  if (isFullScreen())
    delayedFullScreenTimer_.singleShot(100);
}
...
void QGLViewer::postDraw() {
  ...
  if (++fpsCounter_ == maxCounter) {
    ...
    fpsString_ = std::format("{0}Hz", f_p_s_);
    fpsCounter_ = 0;
  }
...
void QGLViewer::setDefaultShortcuts() {
  ...
  setShortcut(ENABLE_TEXT, Qt::ShiftModifier | Qt::Key_Question);
  setShortcut(CAMERA_MODE, Qt::Key_Space);
  setShortcut(FULL_SCREEN, Qt::AltModifier | Qt::Key_Return);
  ...
  keyboardActionDescription_[CAMERA_MODE] = "Changes camera mode (observe or fly)";
...
void QGLViewer::setDefaultMouseBindings() {
  const Qt::KeyboardModifier cameraKeyboardModifiers = Qt::NoModifier;
  const Qt::KeyboardModifier frameKeyboardModifiers = Qt::ControlModifier;
  ...
  const Qt::KeyboardModifier macKeyboardModifiers = Qt::AltModifier;
...
void QGLViewer::setCamera(Camera *const camera) {
  ...
  this->camera()->frame()->disconnect("manipulated", this);             
  this->camera()->frame()->disconnect("spun", this);
  //screen()->disconnect("physicalDotsPerInchChanged", this->camera());
  ...
  camera->frame()->connect("manipulated", std::bind(&QGLViewer::update, this), this);
  camera->frame()->connect("spun", std::bind(&QGLViewer::update, this), this);
  //screen()->connect<void, qreal>("physicalDotsPerInchChanged", std::bind(&Camera::setDevicePixelRatio, camera), camera);
...
void QGLViewer::connectAllCameraKFIInterpolatedSignals(bool connection) {
  for (auto it = camera()->kfi().begin(), end = camera()->kfi().end() ; it != end ; ++it) {
    if (connection)
      camera()->keyFrameInterpolator(it->first)->connect("interpolated", std::bind(&QGLViewer::update, this), this);
    else
      camera()->keyFrameInterpolator(it->first)->disconnect("interpolated", this);
  }

  if (connection)
    camera()->interpolationKfi()->connect("interpolated", std::bind(&QGLViewer::update, this), this);
  else
    camera()->interpolationKfi()->disconnect("interpolated", this);               
}
...
void QGLViewer::renderText(int x, int y, const std::string &str,
                           const QFont &font) {
...
void QGLViewer::renderText(double x, double y, double z, const std::string &str,
                           const QFont &font) {
...
method.
*/
void QGLViewer::drawText(int x, int y, const std::string &text, const QFont &fnt) {
  if (!textIsEnabled())
    return;
...
void QGLViewer::displayMessage(const std::string &message, int delay) {
  message_ = message;
  displayMessage_ = true;
  // Was set to single shot in defaultConstructor.
  messageTimer_.singleShot(delay);
  if (textIsEnabled())
    update();
}
...
void QGLViewer::startAnimation() {
  animationTimer_.start(animationPeriod());
...
static std::string mouseButtonsString(Qt::MouseButton b) {
  std::string result("");
...
void QGLViewer::mousePressEvent(QMouseEvent *e) {
  ...
  ClickBindingPrivate cbp(e->modifiers(), e->button(), false,
                          static_cast<Qt::MouseButton>((e->buttons() & ~(e->button()))),
  ...
  if (clickBinding_.contains(cbp)) {
    ...
    if (mouseGrabberIsAManipulatedFrame_) {
      for (std::map<MouseBindingPrivate, MouseActionPrivate>::const_iterator
               it = mouseBinding_.begin(),
               end = mouseBinding_.end();
           it != end; ++it)
        if ((it->second.handler == FRAME) && (it->first.button == e->button())) {
          ManipulatedFrame *mf =
              dynamic_cast<ManipulatedFrame *>(mouseGrabber());
          if (mouseGrabberIsAManipulatedCameraFrame_) {
            mf->ManipulatedFrame::startAction(it->second.action,
                                              it->second.withConstraint);
            mf->ManipulatedFrame::mousePressEvent(e, camera());
          } else {
            mf->startAction(it->second.action, it->second.withConstraint);
...
void QGLViewer::mouseMoveEvent(QMouseEvent *e) {
  ...
  if (!mouseGrabber()) {
    ...
    if (camera()->frame()->isManipulated()) {
      ...
      // beginning
      if (camera()->frame()->action() == ZOOM_ON_REGION)
        update();
    } else // !
      ...
      for(auto mgit = MouseGrabber::MouseGrabberPool().begin()  ; mgit !=  MouseGrabber::MouseGrabberPool().end() ; ++mgit) {
        (*mgit)->checkIfGrabsMouse(e->x(), e->y(), camera());
        if ((*mgit)->grabsMouse()) {
          setMouseGrabber(*mgit);
          // Check that MouseGrabber is not disabled
          if (mouseGrabber() == *mgit) {
            update();
            break;
          }
        }
...
void QGLViewer::wheelEvent(QWheelEvent *e) {
  if (mouseGrabber()) {
    if (mouseGrabberIsAManipulatedFrame_) {
      for (std::map<WheelBindingPrivate, MouseActionPrivate>::const_iterator
               it = wheelBinding_.begin(),
               end = wheelBinding_.end();
           it != end; ++it)
        if (it->second.handler == FRAME) {
          ManipulatedFrame *mf =
              dynamic_cast<ManipulatedFrame *>(mouseGrabber());
          if (mouseGrabberIsAManipulatedCameraFrame_) {
            mf->ManipulatedFrame::startAction(it->second.action,
...
void QGLViewer::setMouseGrabberIsEnabled(
  ...
  if (enabled)
    disabledMouseGrabbers_.erase(reinterpret_cast<size_t>(mouseGrabber));
  else
    disabledMouseGrabbers_[reinterpret_cast<size_t>(mouseGrabber)];
}

std::string QGLViewer::mouseActionString(QGLViewer::MouseAction ma) {
  switch (ma) {
  ...
  case QGLViewer::SCREEN_ROTATE:
    return "Rotates in screen plane";
  case QGLViewer::ROLL:
    return "Rolls";
  case QGLViewer::DRIVE:
    return "Drives";
  case QGLViewer::SCREEN_TRANSLATE:
    return "Horizontally/Vertically translates";
  case QGLViewer::ZOOM_ON_REGION:
    return "Zooms on region for";
  }
  return std::string();
}

std::string QGLViewer::clickActionString(QGLViewer::ClickAction ca) {
  switch (ca) {
  case QGLViewer::NO_CLICK_ACTION:
    return std::string();
  case QGLViewer::ZOOM_ON_PIXEL:
    return "Zooms on pixel";
  case QGLViewer::ZOOM_TO_FIT:
    return "Zooms to fit scene";
  case QGLViewer::SELECT:
    return "Selects";
  case QGLViewer::RAP_FROM_PIXEL:
    return "Sets pivot point";
  case QGLViewer::RAP_IS_CENTER:
    return "Resets pivot point";
  case QGLViewer::CENTER_FRAME:
    return "Centers manipulated frame";
  case QGLViewer::CENTER_SCENE:
    return "Centers scene";
  case QGLViewer::SHOW_ENTIRE_SCENE:
    return "Shows entire scene";
  case QGLViewer::ALIGN_FRAME:
    return "Aligns manipulated frame";
  case QGLViewer::ALIGN_CAMERA:
    return "Aligns camera";
  }
  return std::string();
}

static std::string keyString(unsigned int key) {
  return QKeySequence(int(key)).toString();
}

std::string QGLViewer::formatClickActionPrivate(ClickBindingPrivate cbp) {
  bool buttonsBefore = cbp.buttonsBefore != Qt::NoButton;
  std::string keyModifierString = keyString(cbp.modifiers | cbp.key);
  if (!keyModifierString.empty()) {
#ifdef Q_OS_MAC
    ...
    if (!keyModifierString.ends_with('+'))
      keyModifierString += "+";
#endif
  }

  return std::format("{0}{1}{2}{3}{4}{5}",
      keyModifierString,
      mouseButtonsString(cbp.button) + (cbp.button == Qt::NoButton ? "Wheel" : ""),
      cbp.doubleClick ? " double click" : "",
      buttonsBefore ? " with ": "",
      buttonsBefore ? mouseButtonsString(cbp.buttonsBefore) : "",
      buttonsBefore ? " pressed" : "");
}
...
/*! This method is deprecated since version 2.5.0
 ...
 Use setMouseBindingDescription(Qt::KeyboardModifier, Qt::MouseButton,
 std::string, bool, Qt::MouseButton) instead.
*/
void QGLViewer::setMouseBindingDescription(unsigned int state,
                                           std::string description,
                                           bool doubleClick,
                                           Qt::MouseButton buttonsBefore) {
  std::cerr << "setMouseBindingDescription(int state,...) is deprecated. Use the modifier/button equivalent" << std::endl;
...
/*! Defines a custom mouse binding description, displayed in the help() window's
 ...
 Same as calling setMouseBindingDescription(Qt::Key, Qt::KeyboardModifier,
 Qt::MouseButton, std::string, bool, Qt::MouseButton), with a key value of
 Qt::Key(0) (i.e. binding description when no regular key needs to be pressed).
 */
void QGLViewer::setMouseBindingDescription(Qt::KeyboardModifier modifiers,
...
\p modifiers is a combination of Qt::KeyboardModifier (\c Qt::ControlModifier,
\c Qt::AltModifier, \c Qt::ShiftModifier, \c Qt::MetaModifier). Possibly
combined using the \c "|" operator.

\p button is one of the Qt::MouseButton (\c Qt::LeftButton, \c Qt::MidButton,
\c Qt::RightButton...).
...
void QGLViewer::setMouseBindingDescription(
    Qt::Key key, Qt::KeyboardModifier modifiers, Qt::MouseButton button,
    std::string description, bool doubleClick, Qt::MouseButton buttonsBefore) {
  ClickBindingPrivate cbp(modifiers, button, doubleClick, buttonsBefore, key);

  if (description.empty())
    mouseDescription_.erase(cbp);
  else
    mouseDescription_[cbp] = description;
}

static std::string tableLine(const std::string &left, const std::string &right) {
  static bool even = false;
  const std::string tdtd("</b></td><td>");
  const std::string tdtr("</td></tr>\n");

  std::string res("<tr bgcolor=\"");
...
/*! Returns a std::string that describes the application mouse bindings, displayed
...
std::string QGLViewer::mouseString() const {
  std::string text("<font color=\"black\"><center><table border=\"0\" cellspacing=\"0\" cellpadding=\"4\">\n");
  const std::string trtd("<tr><td>");
  const std::string tdtr("</td></tr>\n");
  const std::string tdtd("</td><td>");

  text += std::format("<tr bgcolor=\"#aaaacc\"><th align=\"center\">{0}</th><th "
                  "align=\"center\">{1}</th></tr>\n","Button(s)","Description");

  std::map<ClickBindingPrivate, std::string> mouseBinding;

  // User-defined mouse bindings come first.
  for (std::map<ClickBindingPrivate, std::string>::const_iterator
           itm = mouseDescription_.begin(),
           endm = mouseDescription_.end();
       itm != endm; ++itm)
    mouseBinding[itm->first] = itm->second;
  ...
  for (std::map<ClickBindingPrivate, std::string>::const_iterator
    ...
    text += tableLine(formatClickActionPrivate(it->first), it->second);
  }

  // Optional separator line
  if (!mouseBinding.empty()) {
    mouseBinding.clear();
    text += std::format("<tr bgcolor=\"#aaaacc\"><td colspan=2>{0}</td></tr>\n","Standard mouse bindings");
  }
  ...
  for (std::map<MouseBindingPrivate, MouseActionPrivate>::const_iterator
       ...
       itmb != endmb; ++itmb) {
    ClickBindingPrivate cbp(itmb->first.modifiers, itmb->first.button, false,
                            Qt::NoButton, itmb->first.key);

    std::string text = mouseActionString(itmb->second.action);

    if (!text.empty()) {
      switch (itmb->second.handler) {
      case CAMERA:
        text += " camera";
        break;
      case FRAME:
        text += " manipulated frame";
        break;
      }
      if (!(itmb->second.withConstraint))
        text += "*";
    }
    mouseBinding[cbp] = text;
  }

  for (auto itw = wheelBinding_.begin() ; itw != wheelBinding_.end(); ++itw) {
    ClickBindingPrivate cbp(itw->first.modifiers, Qt::NoButton, false, Qt::NoButton, itw->first.key);

    std::string text = mouseActionString(itw->second.action);

    if (!text.empty()) {
      switch (itw->second.handler) {
      case CAMERA:
        text += " camera";
        break;
      case FRAME:
  ...
  for (std::map<ClickBindingPrivate, ClickAction>::const_iterator
           itcb = clickBinding_.begin(),
           endcb = clickBinding_.end();
       itcb != endcb; ++itcb)
    mouseBinding[itcb->first] = clickActionString(itcb->second);
  ...
  for (std::map<ClickBindingPrivate, std::string>::const_iterator
    ...
    text += tableLine(formatClickActionPrivate(it2->first), it2->second);
  }
...
details. */
void QGLViewer::setKeyDescription(unsigned int key, std::string description) {
  if (description.empty())
    keyDescription_.erase(key);
  else
    keyDescription_[key] = description;
}

std::string QGLViewer::cameraPathKeysString() const {
  if (pathIndex_.empty())
    return std::string();

  std::vector<Qt::Key> keys;
  keys.reserve(pathIndex_.size());
  for (std::map<Qt::Key, unsigned int>::const_iterator i = pathIndex_.begin(),
                                                  endi = pathIndex_.end();
       i != endi; ++i)
    keys.push_back(i->first);
  std::sort(keys.begin(), keys.end());

  std::vector<Qt::Key>::const_iterator it = keys.begin(), end = keys.end();
  std::string res = keyString(*it);
...
/*! Returns a std::string that describes the application keyboard shortcut bindings,
...
std::string QGLViewer::keyboardString() const {
  std::string text(
      "<font color=\"black\"><center><table border=\"0\" cellspacing=\"0\" cellpadding=\"4\">\n");
  text += std::format("<tr bgcolor=\"#aaaacc\"><th align=\"center\">{0}</th><th "
                  "align=\"center\">{1}</th></tr>\n","Key(s)","Description");

  std::map<unsigned int, std::string> keyDescription;

  // 1 - User defined key descriptions
  for (std::map<unsigned int, std::string>::const_iterator kd = keyDescription_.begin(),
                                                  kdend = keyDescription_.end();
       kd != kdend; ++kd)
    keyDescription[kd->first] = kd->second;

  // Add to text in sorted order
  for (std::map<unsigned int, std::string>::const_iterator kb = keyDescription.begin(),
                                                  endb = keyDescription.end();
       kb != endb; ++kb)
    text += tableLine(keyString(kb->first), kb->second);
  ...
  for (std::map<KeyboardAction, unsigned int>::const_iterator
           it = keyboardBinding_.begin(),
           end = keyboardBinding_.end();
       it != end; ++it)
    if ((it->second != 0) &&
        ((!cameraIsInRotateMode()) ||
         ((it->first != INCREASE_FLYSPEED) && (it->first != DECREASE_FLYSPEED))))
      keyDescription[it->second] = keyboardActionDescription_.at(it->first);

  // Add to text in sorted order
  for (std::map<unsigned int, std::string>::const_iterator kb2 = keyDescription.begin(),
                                                  endb2 = keyDescription.end();
       kb2 != endb2; ++kb2)
    text += tableLine(keyString(kb2->first), kb2->second);

  // 4 - Camera paths keys description
  const std::string cpks = cameraPathKeysString();
  if (!cpks.empty()) {
    text += "<tr bgcolor=\"#ccccff\"><td colspan=2>\n";
    text += std::format("Camera paths are controlled using the {0} keys (noted <i>Fx</i> below):",cpks) +
            "</td></tr>\n";
    text += tableLine(
...
void QGLViewer::help() {
  ...
  static std::string label[] = {"&Help", "&Keyboard", "&Mouse", "&About"};
 ...
 /* if (!helpWidget()) {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    case 2:
      ...
      break;
    case 3:
      text = std::string("<center><br><img src=\"mydata://qglviewer-icon.xpm\">") +
             std::format("<h1>libQGLViewer</h1>"
                "<h3>Version {0}</h3><br>"
                "A versatile 3D viewer based on OpenGL and Qt<br>"
                "Copyright 2002-{1} Gilles Debunne<br>"
                "<code>{2}</code>",QGLViewerVersionString(),"2023","http://www.libqglviewer.com") +
             std::string("</center>");
      break;
    default:
      break;
...
void QGLViewer::keyPressEvent(QKeyEvent *e) {
  ...
  const Qt::KeyboardModifier modifiers = e->modifiers();

  std::map<KeyboardAction, unsigned int>::const_iterator it = keyboardBinding_.begin(),
                                                   end = keyboardBinding_.end();
  const unsigned int target = key | modifiers;
  while ((it != end) && (it->second != target))
    ++it;

  if (it != end)
    handleKeyboardAction(it->first);
    ...
    if (modifiers == playPathKeyboardModifiers()) {
      ...
      if ((elapsed < 250) && (index == previousPathId_)) {
        if (camera()->keyFrameInterpolator(index)) {
          camera()->keyFrameInterpolator(index)->disconnect("interpolated", this);
          if (camera()->keyFrameInterpolator(index)->numberOfKeyFrames() > 1)
            displayMessage(std::format("Path {0} deleted",index));
          else
            displayMessage(std::format("Position {0} deleted",index));
        ...
        if (nullBefore)
          camera()->keyFrameInterpolator(index)->connect("interpolated", std::bind(&QGLViewer::update, this), this);
        int nbKF = camera()->keyFrameInterpolator(index)->numberOfKeyFrames();
        if (nbKF > 1)
          displayMessage(std::format("Path {0}, position {1} added", index, nbKF));
        else
          displayMessage(std::format("Position {0} saved",index));
      }
      previousPathId_ = index;
    }
    update();
...
unsigned int QGLViewer::shortcut(KeyboardAction action) const {
  if (keyboardBinding_.contains(action))
    return keyboardBinding_.at(action);
  else
    return 0;
}
...
void QGLViewer::setKeyboardAccelerator(KeyboardAction action,
                                       unsigned int key) {
  std::cerr << "setKeyboardAccelerator is deprecated. Use setShortcut instead." << std::endl;
  setShortcut(action, key);
}

unsigned int QGLViewer::keyboardAccelerator(KeyboardAction action) const {
  std::cerr << "keyboardAccelerator is deprecated. Use shortcut instead." << std::endl;
...
Qt::Key QGLViewer::pathKey(unsigned int index) const {
  for (std::map<Qt::Key, unsigned int>::const_iterator it = pathIndex_.begin(),
                                                  end = pathIndex_.end();
       it != end; ++it)
    if (it->second == index)
      return it->first;
...
void QGLViewer::setPlayPathKeyboardModifiers(Qt::KeyboardModifier modifiers) {
...
Qt::KeyboardModifier QGLViewer::addKeyFrameKeyboardModifiers() const {
...
Qt::KeyboardModifier QGLViewer::playPathKeyboardModifiers() const {
  return playPathKeyboardModifiers_;
}

#ifndef DOXYGEN
// Deprecated methods
Qt::KeyboardModifier QGLViewer::addKeyFrameStateKey() const {
  std::cerr << "addKeyFrameStateKey has been renamed addKeyFrameKeyboardModifiers" << std::endl;
  return addKeyFrameKeyboardModifiers();
}

Qt::KeyboardModifier QGLViewer::playPathStateKey() const {
  std::cerr << "playPathStateKey has been renamed playPathKeyboardModifiers" << std::endl;
  return playPathKeyboardModifiers();
}

void QGLViewer::setAddKeyFrameStateKey(unsigned int buttonState) {
  std::cerr << "setAddKeyFrameStateKey has been renamed setAddKeyFrameKeyboardModifiers" << std::endl;
...
void QGLViewer::setPlayPathStateKey(unsigned int buttonState) {
  std::cerr << "setPlayPathStateKey has been renamed setPlayPathKeyboardModifiers" << std::endl;
  setPlayPathKeyboardModifiers(keyboardModifiersFromState(buttonState));
}

Qt::Key QGLViewer::keyFrameKey(unsigned int index) const {
  std::cerr << "keyFrameKey has been renamed pathKey." << std::endl;
  return pathKey(index);
}

Qt::KeyboardModifier QGLViewer::playKeyFramePathStateKey() const {
  std::cerr << "playKeyFramePathStateKey has been renamed playPathKeyboardModifiers." << std::endl;
  return playPathKeyboardModifiers();
}

void QGLViewer::setKeyFrameKey(unsigned int index, int key) {
  std::cerr << "setKeyFrameKey is deprecated, use setPathKey instead, with swapped parameters." << std::endl;
  setPathKey(key, index);
}

void QGLViewer::setPlayKeyFramePathStateKey(unsigned int buttonState) {
  std::cerr << "setPlayKeyFramePathStateKey has been renamed setPlayPathKeyboardModifiers." << std::endl;
  setPlayPathKeyboardModifiers(keyboardModifiersFromState(buttonState));
}
#endif
...
void QGLViewer::setHandlerKeyboardModifiers(MouseHandler handler,
                                            Qt::KeyboardModifier modifiers) {
  std::cerr << "setHandlerKeyboardModifiers is deprecated, call setMouseBinding() instead" << std::endl;

  std::map<MouseBindingPrivate, MouseActionPrivate> newMouseBinding;
  std::map<WheelBindingPrivate, MouseActionPrivate> newWheelBinding;
  std::map<ClickBindingPrivate, ClickAction> newClickBinding_;

  std::map<MouseBindingPrivate, MouseActionPrivate>::iterator mit;
  std::map<WheelBindingPrivate, MouseActionPrivate>::iterator wit;

  // First copy unchanged bindings.
  for (mit = mouseBinding_.begin(); mit != mouseBinding_.end(); ++mit)
    if ((mit->second.handler != handler) ||
        (mit->second.action == ZOOM_ON_REGION))
      newMouseBinding[mit->first] = mit->second;

  for (wit = wheelBinding_.begin(); wit != wheelBinding_.end(); ++wit)
    if (wit->second.handler != handler)
      newWheelBinding[wit->first] = wit->second;

  // Then, add modified bindings, that can overwrite the previous ones.
  for (mit = mouseBinding_.begin(); mit != mouseBinding_.end(); ++mit)
    if ((mit->second.handler == handler) &&
        (mit->second.action != ZOOM_ON_REGION)) {
      MouseBindingPrivate mbp(modifiers, mit->first.button, mit->first.key);
      newMouseBinding[mbp] = mit->second;
    }

  for (wit = wheelBinding_.begin(); wit != wheelBinding_.end(); ++wit)
    if (wit->second.handler == handler) {
      WheelBindingPrivate wbp(modifiers, wit->first.key);
      newWheelBinding[wbp] = wit->second;
    }
  ...
  for (std::map<ClickBindingPrivate, ClickAction>::const_iterator
       ...
       cb != end; ++cb)
    if (((handler == CAMERA) &&
         ((cb->second == CENTER_SCENE) || (cb->second == ALIGN_CAMERA))) ||
        ((handler == FRAME) &&
         ((cb->second == CENTER_FRAME) || (cb->second == ALIGN_FRAME)))) {
      ClickBindingPrivate cbp(modifiers, cb->first.button, cb->first.doubleClick,
                              cb->first.buttonsBefore, cb->first.key);
      newClickBinding_[cbp] = cb->second;
    } else
      newClickBinding_[cb->first] = cb->second;

  mouseBinding_.clear();
  for (auto it = newMouseBinding.begin() ; it!=newMouseBinding.end() ; ++it)
    mouseBinding_[it->first] = it->second;
...
void QGLViewer::setHandlerStateKey(MouseHandler handler,
  ...
  std::cerr << "setHandlerStateKey has been renamed setHandlerKeyboardModifiers" << std::endl;
...
void QGLViewer::setMouseStateKey(MouseHandler handler,
                                 unsigned int buttonState) {
  std::cerr << "setMouseStateKey has been renamed setHandlerKeyboardModifiers." << std::endl;
  setHandlerKeyboardModifiers(handler, keyboardModifiersFromState(buttonState));
}

/*! This method is deprecated since version 2.5.0

 Use setMouseBinding(Qt::KeyboardModifier, Qt::MouseButton, MouseHandler,
...
void QGLViewer::setMouseBinding(unsigned int state, MouseHandler handler,
  ...
  std::cerr << "setMouseBinding(int state, MouseHandler...) is deprecated. Use the modifier/button equivalent" << std::endl;
...
/*! Defines a MouseAction binding.
  ...
  Same as calling setMouseBinding(Qt::Key, Qt::KeyboardModifier,
  ...
  action). */
void QGLViewer::setMouseBinding(Qt::KeyboardModifier modifiers,
...
setMouseBinding(Qt::KeyboardModifier, Qt::MouseButton, MouseHandler,
MouseAction, bool)).
...
See also setMouseBinding(Qt::KeyboardModifier, Qt::MouseButton, ClickAction,
bool, int), setWheelBinding() and clearMouseBindings(). */
void QGLViewer::setMouseBinding(Qt::Key key, Qt::KeyboardModifier modifiers,
                                Qt::MouseButton button, MouseHandler handler,
                                MouseAction action, bool withConstraint) {
  if ((handler == FRAME) &&
    ...
    std::cerr << "Cannot bind " << mouseActionString(action) << " to FRAME" << std::endl;
    return;
  }

  if (button == Qt::NoButton) {
    std::cerr << "No mouse button specified in setMouseBinding" << std::endl;
    return;
  }
  ...
  if (action == NO_MOUSE_ACTION)
    mouseBinding_.erase(mbp);
  else
    mouseBinding_[mbp] = map;

  ClickBindingPrivate cbp(modifiers, button, false, Qt::NoButton, key);
  clickBinding_.erase(cbp);
}
...
void QGLViewer::setMouseBinding(unsigned int state, ClickAction action,
                                bool doubleClick,
                                Qt::MouseButton buttonsBefore) {
  std::cerr << "setMouseBinding(int state, ClickAction...) is deprecated. Use the modifier/button equivalent" << std::endl;
  setMouseBinding(keyboardModifiersFromState(state),
...
/*! Defines a ClickAction binding.
 ...
 Same as calling setMouseBinding(Qt::Key, Qt::KeyboardModifier,
 Qt::MouseButton, ClickAction, bool, Qt::MouseButton), with a key value of
...
See also setMouseBinding(Qt::KeyboardModifier, Qt::MouseButton, MouseHandler,
...
void QGLViewer::setMouseBinding(Qt::Key key, Qt::KeyboardModifier modifiers,
  ...
  if ((buttonsBefore != Qt::NoButton) && !doubleClick) {
    std::cerr << "Buttons before is only meaningful when doubleClick is true in setMouseBinding()." << std::endl;
    return;
  }
...
/*! Defines a mouse wheel binding.
 ...
 Same as calling setWheelBinding(Qt::Key, Qt::KeyboardModifier, MouseHandler,
 MouseAction, bool), with a key value of Qt::Key(0) (i.e. no regular key needs
 to be pressed to activate this action). */
void QGLViewer::setWheelBinding(Qt::KeyboardModifier modifiers,
...
void QGLViewer::setWheelBinding(Qt::Key key, Qt::KeyboardModifier modifiers,
                                MouseHandler handler, MouseAction action,
                                bool withConstraint) {
  ...
  if ((action != ZOOM) && (action != MOVE_FORWARD) &&
    ...
    std::cerr << "Cannot bind " <<  mouseActionString(action) << " to wheel" << std::endl;            
    return;
  }

  if ((handler == FRAME) && (action != ZOOM) && (action != NO_MOUSE_ACTION)) {
    std::cerr << "Cannot bind " << mouseActionString(action) << " to FRAME wheel" << std::endl;             
    return;
  }
...
/*! This method is deprecated since version 2.5.0
 ...
 Use mouseAction(Qt::Key, Qt::KeyboardModifier, Qt::MouseButton) instead.
*/
QGLViewer::MouseAction QGLViewer::mouseAction(unsigned int state) const {
  std::cerr << "mouseAction(int state,...) is deprecated. Use the modifier/button equivalent" << std::endl;
...
QGLViewer::MouseAction QGLViewer::mouseAction(Qt::Key key,
  ...
  MouseBindingPrivate mbp(modifiers, button, key);
  if (mouseBinding_.contains(mbp))
    return mouseBinding_.at(mbp).action;
  else
...
/*! This method is deprecated since version 2.5.0
 ...
 Use mouseHanler(Qt::Key, Qt::KeyboardModifier, Qt::MouseButton) instead.
*/
int QGLViewer::mouseHandler(unsigned int state) const {
  std::cerr << "mouseHandler(int state,...) is deprecated. Use the modifier/button equivalent" << std::endl;
...
int QGLViewer::mouseHandler(Qt::Key key, Qt::KeyboardModifier modifiers,
                            Qt::MouseButton button) const {
  MouseBindingPrivate mbp(modifiers, button, key);
  if (mouseBinding_.contains(mbp))
    return mouseBinding_.at(mbp).handler;
  else
    return -1;
}
...
int QGLViewer::mouseButtonState(MouseHandler handler, MouseAction action,
  ...
  std::cerr << "mouseButtonState() is deprecated. Use mouseButtons() and keyboardModifiers() instead" << std::endl;
  for (std::map<MouseBindingPrivate, MouseActionPrivate>::const_iterator
           it = mouseBinding_.begin(),
           end = mouseBinding_.end();
       it != end; ++it)
    if ((it->second.handler == handler) && (it->second.action == action) &&
        (it->second.withConstraint == withConstraint))
      return static_cast<int>(it->first.modifiers) | static_cast<int>(it->first.button);
...
QGLViewer::wheelAction(Qt::Key key, Qt::KeyboardModifier modifiers) const {
  WheelBindingPrivate wbp(modifiers, key);
  if (wheelBinding_.contains(wbp))
    return wheelBinding_.at(wbp).action;
  else
    return NO_MOUSE_ACTION;
}
...
int QGLViewer::wheelHandler(Qt::Key key,
  ...
  WheelBindingPrivate wbp(modifiers, key);
  if (wheelBinding_.contains(wbp))
    return wheelBinding_.at(wbp).handler;
  else
    return -1;
}
...
QGLViewer::clickAction(Qt::Key key, Qt::KeyboardModifier modifiers,
                       Qt::MouseButton button, bool doubleClick,
                       Qt::MouseButton buttonsBefore) const {
  ClickBindingPrivate cbp(modifiers, button, doubleClick, buttonsBefore, key);
  if (clickBinding_.contains(cbp))
    return clickBinding_.at(cbp);
  else
...
/*! This method is deprecated since version 2.5.0
  ...
  Use wheelAction(Qt::Key key, Qt::KeyboardModifier modifiers) instead. */
QGLViewer::MouseAction
QGLViewer::wheelAction(Qt::KeyboardModifier modifiers) const {
  std::cerr << "wheelAction() is deprecated. Use the new wheelAction() method with a key parameter instead" << std::endl;
  return wheelAction(Qt::Key(0), modifiers);
}

/*! This method is deprecated since version 2.5.0

  Use wheelHandler(Qt::Key key, Qt::KeyboardModifier modifiers) instead. */
int QGLViewer::wheelHandler(Qt::KeyboardModifier modifiers) const {
  std::cerr << "wheelHandler() is deprecated. Use the new wheelHandler() method with a key parameter instead" << std::endl;
...
unsigned int QGLViewer::wheelButtonState(MouseHandler handler,
                                         MouseAction action,
                                         bool withConstraint) const {
  std::cerr << "wheelButtonState() is deprecated. Use the wheelAction() and wheelHandler() instead" << std::endl;
  for (std::map<WheelBindingPrivate, MouseActionPrivate>::const_iterator
           it = wheelBinding_.begin(),
           end = wheelBinding_.end();
       it != end; ++it)
    if ((it->second.handler == handler) && (it->second.action == action) &&
        (it->second.withConstraint == withConstraint))
      return it->first.key + it->first.modifiers;
...
/*! This method is deprecated since version 2.5.0
 ...
 Use clickAction(Qt::KeyboardModifier, Qt::MouseButton, bool,
 Qt::MouseButton) instead.
*/
QGLViewer::ClickAction
QGLViewer::clickAction(unsigned int state, bool doubleClick,
                       Qt::MouseButton buttonsBefore) const {
  std::cerr << "clickAction(int state,...) is deprecated. Use the modifier/button equivalent" << std::endl;
...
/*! This method is deprecated since version 2.5.0
 ...
 Use getClickActionState(ClickAction, Qt::Key, Qt::KeyboardModifier,
 Qt::MouseButton, bool, Qt::MouseButton) instead.
*/
void QGLViewer::getClickButtonState(ClickAction action, unsigned int &state,
                                    bool &doubleClick,
                                    Qt::MouseButton &buttonsBefore) const {
  std::cerr << "getClickButtonState(int state,...) is deprecated. Use the modifier/button equivalent" << std::endl;
  Qt::KeyboardModifier modifiers;
  Qt::MouseButton button;
  Qt::Key key;
...
void QGLViewer::getClickActionBinding(ClickAction action, Qt::Key &key,
  ...
  for (std::map<ClickBindingPrivate, ClickAction>::const_iterator
    ...
    if (it->second == action) {
      modifiers = it->first.modifiers;
      button = it->first.button;
      doubleClick = it->first.doubleClick;
      buttonsBefore = it->first.buttonsBefore;
      key = it->first.key;
      return;
    }
...
void QGLViewer::setManipulatedFrame(ManipulatedFrame *frame) {
  if (manipulatedFrame()) {
    ...
    if (manipulatedFrame() != camera()->frame()) {
      manipulatedFrame()->disconnect("manipulated", this);
      manipulatedFrame()->disconnect("spun", this);
    }
  }
  ...
  if (manipulatedFrame()) {
    ...
    // updates
    if (manipulatedFrame() != camera()->frame()) {
      manipulatedFrame()->connect("manipulated", std::bind(&QGLViewer::update, this), this);
      manipulatedFrame()->connect("spun", std::bind(&QGLViewer::update, this), this);
    }
  }
}
...
void QGLViewer::drawVisualHints() {
  ...
  Vec pnt;
  if (camera()->frame()->action() == SCREEN_ROTATE) {
    mf = camera()->frame();
    pnt = camera()->pivotPoint();
  }
  if (manipulatedFrame() && (manipulatedFrame()->action() == SCREEN_ROTATE)) {
    mf = manipulatedFrame();
  ...
  if (mf) {
    ...
    glVertex2i(mf->prevPos().x(), mf->prevPos().y());
    glEnd();
    glEnable(GL_DEPTH_TEST);
    stopScreenCoordinatesSystem();
  }
  ...
  if (camera()->frame()->action() == ZOOM_ON_REGION) {
    ...
    glVertex2i(camera()->frame()->pressPos().x(),
               camera()->frame()->pressPos().y());
    glVertex2i(camera()->frame()->prevPos().x(),
               camera()->frame()->pressPos().y());
    glVertex2i(camera()->frame()->prevPos().x(),
               camera()->frame()->prevPos().y());
    glVertex2i(camera()->frame()->pressPos().x(),
               camera()->frame()->prevPos().y());
    glEnd();
    glEnable(GL_DEPTH_TEST);
...
void QGLViewer::setVisualHintsMask(int mask, int delay) {
  ...
  resetVisualHintsTimer_.singleShot(delay);
}
