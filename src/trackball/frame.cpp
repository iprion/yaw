...
Frame::Frame() : Signaler({"modified"}), constraint_(nullptr), referenceFrame_(nullptr) {}
...
Frame::Frame(const Vec &position, const Quaternion &orientation)
    : Signaler({"modified"}), t_(position), q_(orientation), constraint_(nullptr), referenceFrame_(nullptr) {}
...
/*! Copy constructor.
  ...
  pointers are copied. */
Frame::Frame(const Frame &frame) :Signaler({"modified"}) { 
  (*this) = frame; 
}
...
/*! Sets the Frame from an OpenGL matrix representation (rotation in the upper
 ...
 Calls onModifiedCallBack(). See also matrix(), getMatrix() and
...
/*! Translates the Frame of \p t (defined in the Frame coordinate system).
  ...
  See also rotate(const Quaternion&). Calls onModifiedCallBack(). */
...
/*! Rotates the Frame by \p q (defined in the Frame coordinate system): R = R*q.
  ...
  See also translate(const Vec&). Calls onModifiedCallBack(). */
...
Only one onModifiedCallBack() is called, which is convenient if this signal is
...
/*! Same as setTranslation(), but \p translation is modified so that the
  ...
  Calls onModifiedCallBack(). See also setRotationWithConstraint() and
...
/*! Same as setRotation(), but \p rotation is modified so that the potential
  ...
  Calls onModifiedCallBack(). See also setTranslationWithConstraint() and
...
/*! Same as setTranslationAndRotation(), but \p translation and \p orientation
  are modified to satisfy the constraint(). Calls onModifiedCallBack(). */
...
modified to satisfy the constraint. Calls onModifiedCallBack(). */
...
Calls onModifiedCallBack() if \p refFrame differs from the current
referenceFrame().
